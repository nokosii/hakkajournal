...
import { BookOpenCheck, Check, FilePenLine, LoaderCircle, Plus, Send, Users } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Textarea } from '@/components/ui/textarea';

export type Submission = { id: string; title: string; titleEn: string | null; authorName: string; abstract: string; abstractEn: string | null; keywords: string | null; status: string; articleBody: string; pages: string | null; doi: string | null; finalName: string | null; issueId: string | null; editorNotes: string; reviewCount: number; averageScore: number | null; createdAt: string };
...
export function EditorDashboard({ initialSubmissions, initialIssues, initialMembers }: { initialSubmissions: Submission[]; initialIssues: Issue[]; initialMembers: Member[] }) {
  ...
  async function setMemberRole(member: Member, role: 'member' | 'editor') {
    setSaving(true); setMessage('');
  ...
  return <section className="editor-dashboard">
    ...
    {members.length > 0 && <section className="member-manager"><div className="section-heading"><div><p className="eyebrow">MEMBER ROLES</p><h2>會員與編輯權限</h2></div><Users /></div><div className="member-admin-list">{members.map((member) => <article key={member.id}><div><b>{member.displayName}</b><span>{member.email}{member.affiliation ? ` · ${member.affiliation}` : ''}</span><small>{member.expertise || '尚未填寫專長'}</small></div><select aria-label={`設定 ${member.displayName} 的角色`} value={member.role} disabled={member.role === 'editor_in_chief' || saving} onChange={(event) => setMemberRole(member, event.target.value as 'member' | 'editor')}><option value="member">會員</option><option value="editor">編輯</option>{member.role === 'editor_in_chief' && <option value="editor_in_chief">主編</option>}</select></article>)}</div></section>}
  </section>;
}
