...
export function SubmissionForm({ memberName, memberAffiliation, assisted = false }: { memberName: string; memberAffiliation: string; assisted?: boolean }) {
  ...
  if (submissionId) return (
    <div className="submission-success"><span><Check /></span><p className="eyebrow">PREPRINT PUBLISHED</p><h2>{assisted ? '特殊作者預印本已代為發布' : '預印本已發布並開放審查'}</h2><p>稿件編號為</p><strong>{submissionId}</strong><p>系統已保存稿件與公開紀錄，會員現在可以閱讀並提交審查。</p><Button onClick={() => { window.location.href = `/preprints/${submissionId}`; }}>查看預印本</Button></div>
  );
  ...
  return (
    <form className="submission-form" onSubmit={onSubmit}>
      ...
      <section className="form-section"><div className="form-number">02</div><div><h2>作者資訊</h2><p>{assisted ? '請依 EMAIL 來稿內容登錄作者資料；作者信箱只供編輯聯絡，不會公開。' : '通訊作者由目前登入的會員帳號帶入；姓名將與預印本公開。'}</p></div>
        <div className="field-grid"><label><span>通訊作者姓名 *</span><Input name="authorName" required defaultValue={assisted ? '' : memberName} /></label>{assisted && <label><span>通訊作者 EMAIL *</span><Input name="authorEmail" type="email" required placeholder="author@example.edu" /></label>}<label className="full-field"><span>服務單位</span><Input name="affiliation" defaultValue={assisted ? '' : memberAffiliation} placeholder="學校／機構、系所與職稱" /></label></div>
      </section>
      <section className="form-section"><div className="form-number">03</div><div><h2>預印本上傳與公開聲明</h2><p>請上傳可供社群閱讀與審查的版本。</p></div>
        ...
        <input type="hidden" name="submissionChannel" value={assisted ? 'assisted_email' : 'member'} />
        <label className="declaration"><input type="checkbox" name="openReviewConsent" required /><span>{assisted ? '我已確認來稿資料完整，且作者同意公開作者身分、預印本、審查意見、作者回覆與編輯推薦紀錄。' : '本人確認稿件未一稿多投，並同意公開作者身分、預印本、審查意見、作者回覆與編輯推薦紀錄。'}</span></label>
      </section>
      {error && <p className="form-error" role="alert">{error}</p>}
      <div className="form-submit"><div><b>送出前請再次確認</b><p>送出後將產生稿件編號，檔案與資料會安全保存。</p></div><Button type="submit" size="lg" disabled={submitting}>{submitting ? <><LoaderCircle className="spin" /> 正在送出</> : <><Send /> {assisted ? '代為發布預印本' : '正式送出稿件'}</>}</Button></div>
    </form>
  );
}
